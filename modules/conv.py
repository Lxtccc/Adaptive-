...
class _ConvNd(Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride,
                 padding, dilation, transposed, output_padding, groups, bias, alpha):
...
class Conv2d(_ConvNd):
    ...
    * :attr:`groups` controls the connections between inputs and outputs.
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=True, alpha=None):
        ...
        super(Conv2d, self).__init__(
            in_channels, out_channels, kernel_size, stride, padding, dilation,
            False, _pair(0), groups, bias, alpha)

    def forward(self, input):
        return F.conv2d(input, self.weight, self.bias, self.stride,
                        self.padding, self.dilation, self.groups)

class Conv2d_with_parameters(_ConvNd):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=True, alpha=None):
        kernel_size = _pair(kernel_size)
        stride = _pair(stride)
        padding = _pair(padding)
        dilation = _pair(dilation)
        super(Conv2d_with_parameters, self).__init__(
            in_channels, out_channels, kernel_size, stride, padding, dilation,
            False, _pair(0), groups, bias, alpha)
        # self.alpha = alpha
        alpha =_pair(alpha)
        self.lamda = torch.nn.Parameter(torch.ones(out_channels), requires_grad=True)
        if self.alpha is None:
            self.alpha = torch.nn.Parameter(torch.ones(out_channels), requires_grad=True)

    def forward(self, input):
        # alpha = F.softmax(self.alpha, dim=-1)
        # weight =self.weight *  torch.nn.Parameter(self.alpha.unsqueeze(2).unsqueeze(3))
        # weight =self.weight *  F.softmax(self.alpha, dim=0)
        out = F.conv2d(input, self.weight, self.bias, self.stride,
                        self.padding, self.dilation, self.groups)
        p = self.alpha.unsqueeze(0).unsqueeze(2).unsqueeze(3)
        return out * p
