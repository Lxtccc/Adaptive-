...
def cal_alpha_var_loss(alpha):
    loss = []
    for a in alpha:
        loss.append(torch.var(a))
    return torch.mean(torch.tensor(loss)).cuda()
    ...
    for k, v in ori_graph_paras.items():
        if k in sub_graph_paras and len(v.shape) == 4:
            ...
            if 'downsample' in k:
                ...
                pre_selected = selected
                ind += 1
        elif k in sub_graph_paras and 'alpha' in k:
            selected = selected_index[ind]
            sub_graph_paras[k] = v[selected]
...
def sample_from_alpha_adaptive(alpha, keep_rate, with_noise):
    ...
    # keep_rate_list =  torch.normal(mean=keep_rate, std=0.1 * torch.ones(len(alpha))).cuda()
    for i, paras in enumerate(alpha):
        paras = paras
        ...
        reduced_len, summ = compute_len(sorted_alpha[0], keep_rate[i])
    ...
    return sampler_ind, sampler_alpha, sampler_len, keep_rate

def update_model_from_subgraph(sub_grah, ori_graph, selected_index):
    ori_graph_paras = ori_graph.state_dict()
    sub_graph_paras = sub_grah.state_dict()
